...
class DigilanTokenSocialProviderGoogle extends DigilanTokenSocialProvider
{
        ...
        case 'client_secret':
          ...
          if (empty($newData[$key])) {
            \DLT\Notices::addError(
              sprintf(
                __('The %1$s entered did not appear to be a valid. Please enter a valid %2$s.', 'digilan-token'),
  ...
  protected function getCurrentUserInfo()
  {
    ...
    return $this->getClient()->get(
      'userinfo?fields=' . implode(',',array_merge($fields, $extra_me_fields))
    );
  }
  ...
  public function getMyGooglePlus()
  {
    ...
    if (!empty($extra_gplus_fields)) {
      return $this->getClient()->get(
        'people/me?fields=' . implode(',', $extra_gplus_fields),
        array(),
  ...
  public function getSyncDataFieldDescription($fieldName)
  {
    if (isset($this->sync_fields[$fieldName]['description'])) {
      return sprintf(
        __('Required API: %1$s', 'digilan-token'),
        $this->sync_fields[$fieldName]['description']
      );
    }
