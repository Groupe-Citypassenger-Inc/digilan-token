        ...
        <table class="form-table">
            <tbody>
                <?php if (!defined('DIGILAN_FB_APP_ID')) : ?>
                    <tr>
                        <th scope="row"><label for="appid"><?php _e('App ID', 'digilan-token'); ?>
                                - <em>(<?php _e('Required', 'digilan-token'); ?>)</em></label></th>
                        <td>
                            <input name="appid" type="text" id="appid" value="<?php echo esc_attr($settings->get('appid')); ?>" class="regular-text">
                            <p class="description" id="tagline-appid"><?php printf(__('If you are not sure what is your %1$s, please head over to <a href="%2$s">Getting Started</a>', 'digilan-token'), 'App ID', $this->getUrl()); ?></p>
                        </td>
                    </tr>
                <?php endif; ?>
                <?php if (!defined('DIGILAN_FB_APP_SECRET')) : ?>
                    <tr>
                        <th scope="row"><label for="secret"><?php _e('App Secret', 'digilan-token'); ?>
                                - <em>(<?php _e('Required', 'digilan-token'); ?>)</em></label>
                        </th>
                        <td><input name="secret" type="text" id="secret" value="<?php echo esc_attr($settings->get('secret')); ?>" class="regular-text"></td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
