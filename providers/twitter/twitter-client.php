...
*/
require_once DLT_PATH . '/includes/digilan-auth.php';

class DigilanTokenSocialProviderTwitterClient extends DigilanTokenSocialAuth
{
  ...
  {
    \DLT\Persistent\Persistent::delete($this->providerID . '_request_token');
  }
  ...
  public function createAuthUrl()
  {
    ...
    \DLT\Persistent\Persistent::set($this->providerID . '_request_token', maybe_serialize($oauthTokenData));
    set_transient('digilan_twitter_connect', maybe_serialize($oauthTokenData));
    $state = $queries['session_id'] . $queries['mac'];
    $transient_name = "digilan_token_twitter_oauth_" . $oauthTokenData['oauth_token'];
    set_transient($transient_name, $state, 1800);
    return $this->endpoint . 'oauth/authenticate?oauth_token=' . $oauthTokenData['oauth_token'];
  }
  ...
  {
    $oauth_token = DigilanTokenSanitize::sanitize_request('oauth_token');
    $oauth_verifier = DigilanTokenSanitize::sanitize_request('oauth_verifier');
  ...
  {
    $requestToken = maybe_unserialize(\DLT\Persistent\Persistent::get($this->providerID . '_request_token'));
    if ($requestToken == '' && $this->providerID == 'twitter') {
      $requestToken = maybe_unserialize(get_transient('digilan_twitter_connect'));
    }
    $response = $this->oauthRequest($this->endpoint . 'oauth/access_token', 'POST', array(), array(
      'oauth_verifier' => DigilanTokenSanitize::sanitize_get('oauth_verifier')
    ), array(
  ...
  private function oauthRequest($url, $method, $_requestData = array(), $_oauthData = array(), $context = array())
  {
    ...
    if (is_wp_error($request)) {
      ...
      throw new Exception(sprintf(__('Unexpected response: %s', 'digilan-token'), wp_remote_retrieve_body($request)));
    }
