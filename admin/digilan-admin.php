...
class DigilanTokenAdmin
{
  ...
  public static function admin_init()
  {
    ...
    add_action(
      'wp_ajax_digilan-token-update-custom-portal-languages-available',
      'DigilanTokenAdmin::update_custom_portal_language_available'
    );
    ...
    if (!function_exists('json_decode')) {
      add_settings_error(
        'digilan-token-social',
        'settings_updated',
        printf(__('%s needs json_decode function.', 'digilan-token'), 'Digilan Token') . ' ' . __('Please contact your server administrator and ask for solution!', 'digilan-token'),
        'error'
      );
    }
  }
  ...
  private static function add_field_to_form()
  {
    ...
    foreach($form_languages as $lang) {
      ...
      $new_field_data = self::add_field_properties_translation_values(
        $new_field_data,
        $lang,
        $safe_field_type,
        'digilan-token-new-field',
      );
    }

    $first_translation_name = current(array_filter($new_field_data['display-name']));
    // Create field key based on field display name
    $new_field_key = str_replace(' ', '_', strtolower($first_translation_name));
  ...
  private static function update_user_form_fields()
  {
    ...
    foreach($user_form_fields as $form_field_key=>$form_field_value) {
      ...
      foreach($form_languages as $lang) {
        ...
        $user_form_fields[$form_field_key] = self::add_field_properties_translation_values(
          $user_form_fields[$form_field_key],
          $lang,
          $field_type,
          "form-fields/$form_field_key",
          $form_field_key,
        );
      }
    }
    ...
    if ('settings_page_digilan-token-plugin' === get_current_screen()->id) {
      ...
      if (function_exists('wp_enqueue_code_editor')) {
        ...
        wp_add_inline_script(
          'code-editor',
          sprintf(
            'jQuery( function() { var settings = %s; jQuery(".digilan-token-html-editor").each(function(i, el){wp.codeEditor.initialize( el, settings);}); } );',
        ...
        wp_add_inline_script(
          'code-editor',
          sprintf(
            'jQuery( function() { var settings = %s; jQuery(".digilan-token-html-editor-readonly").each(function(i, el){wp.codeEditor.initialize( el, settings);}); } );',
            wp_json_encode($settings)
          )
        );
      }
