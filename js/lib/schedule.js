(function ($) {
  ...
  DayScheduleSelector.prototype.attachEvents = function () {
    ...
    this.$el.on('click', '.time-slot', function () {
      ...
      if (!plugin.isSelecting()) {  // if we are not in selecting mode
        ...
        if (isSlotSelected(plugin.$selectingStart) == true) {
          plugin.deselect(plugin.$el.find(plugin.$selectingStart));
        }
        if (day == plugin.$selectingStart.data('day')) {  // if clicking on the same day column
          ...
          plugin.$el.find('.time-slot[data-day="' + day + '"]').filter('[data-deselect]')
            .removeAttr('data-selected');
          plugin.$el.find('.time-slot[data-day="' + day + '"]').filter('[data-deselect]')
            .removeAttr('data-deselect');
          plugin.$el.find('.time-slot').removeAttr('data-disabled');
          plugin.$el.trigger('selected.dayScheduleSelector', [getSelection(plugin, plugin.$selectingStart, $(this))]);
    ...
    this.$el.on('mouseover', '.time-slot', function () {
      ...
      if (plugin.isSelecting()) {  // if we are in selecting mode
        ...
        if ($slots[start].attributes.getNamedItem('data-selected') !== null) {
          $slots.filter('[data-deselect]').removeAttr('data-deselect').removeAttr('data-selecting');
        } else {
        ...
        if (start > end) {
          ...
          if ($slots[end].attributes.getNamedItem('data-selected') !== null) {
            if ($slots[end].attributes.getNamedItem('data-selected').nodeValue == 'selected') {
              $slots.slice(start, end + 1).attr('data-deselect', 'deselect');
            }
          } else {
          ...
          if ($slots[start].attributes.getNamedItem('data-selected') !== null) {
            if ($slots[start].attributes.getNamedItem('data-selected').nodeValue == 'selected') {
              $slots.slice(start, end + 1).attr('data-deselect', 'deselect');
            }
          } else {
            $slots.slice(start, end + 1).attr('data-selecting', 'selecting');
  ...
  DayScheduleSelector.prototype.deserialize = function (schedule) {
    ...
    $.each(schedule, function (d, ds) {
      ...
      // Clear all selected slots when rendering
  ...
  function Plugin(option) {
    return this.each(function () {
      var $this = $(this)
        , data = $this.data('dayScheduleSelector')
        , options = typeof option == 'object' && option;

      if (!data) {
        $this.data('dayScheduleSelector', (data = new DayScheduleSelector(this, options)));
      }
    })
  }
