...
abstract class DigilanTokenSocialOauth2 extends DigilanTokenSocialAuth
{
  ...
  {
    $code = DigilanTokenSanitize::sanitize_request('code');
    if ($code) {
      return true;
    }
    return false;
  ...
  public function createAuthUrl()
  {
    ...
    $scopes = apply_filters('dlt_' . $this->providerID . '_scopes', $this->scopes);
    if (count($scopes)) {
  ...
  /**
   *
   * @return bool|false|string If the code that was sent by the selected provider and the state is valid,
  ...
  {
    $code = DigilanTokenSanitize::sanitize_get('code');
    if ($code) {
      if (!$this->validateState()) {
  ...
  {
    \DLT\Persistent\Persistent::delete($this->providerID . '_state');
  }
  ...
  {
    $this->state = \DLT\Persistent\Persistent::get($this->providerID . '_state');
    if ($this->state === false) {
      return false;
    }
  ...
  protected function getState()
  {
    ...
    \DLT\Persistent\Persistent::set($this->providerID . '_state', $this->state);
    return $this->state;
  }
