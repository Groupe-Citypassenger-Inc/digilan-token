...
abstract class DigilanTokenSocialProvider extends DigilanTokenSocialProviderDummy
{
    ...
    public function getConnectButton($buttonStyle = 'default', $redirectTo = null, $formFieldsIn)
    {
        ...
        $button = '<a href="' . esc_url(add_query_arg($arg, $this->getLoginUrl($formFieldsIn))) . '" class="dlt-auth" rel="nofollow" aria-label="' . esc_attr__($this->settings->get('login_label')) . '" data-plugin="dlt" data-action="connect" data-popupwidth="' . $this->getPopupWidth() . '" data-popupheight="' . $this->getPopupHeight() . '">' . $button . '</a>';
        return $button;
    }
