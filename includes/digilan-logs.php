...
class DigilanTokenLogs
{
  ...
  public static function generate_csv($datetime_start, $datetime_end)
  {
    ...
    ON {$wpdb->prefix}digilan_token_users_$version.id = {$wpdb->prefix}digilan_token_logs_archive.user_id
    ...
    LEFT JOIN {$wpdb->prefix}digilan_token_users_$version
    ON {$wpdb->prefix}digilan_token_users_$version.id = {$wpdb->prefix}digilan_token_logs.user_id
    WHERE date >= '%s' AND date <= '%s'";
    global $wpdb;
    $safe_query = $wpdb->prepare($query, array(
      $datetime_start . ' 00:00:00',
